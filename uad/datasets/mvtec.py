...
class MVTecDataset(Dataset):
    def __init__(self,
        ...
        if not is_predict and is_train:
            ...
            self.x, self.y, self.mask = self.load_dataset_folder_test()
    ...
    def load_dataset_folder_test(self):
        phase = 'train' if self.is_train else 'test'
        x, y, mask = [], [], []
...
class MVTecDatasetSTFPM_torch(Dataset):
    ...
    for sb in sub_dirs:
        for fname in sorted(os.listdir(os.path.join(gt_dir, sb))):
            ...
            temp = cv2.resize(temp, (
                resize_shape[0], resize_shape[1])).astype(np.bool_)[None, ...]
            gt.append(temp)
